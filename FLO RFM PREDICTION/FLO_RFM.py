...
df.groupby("order_channel").agg({"master_id": "count",
                                 "New_order_num_total": "sum",
                                 "New_customer_value_total": "sum"})

# Görselleştirme:
# sns.catplot(x="order_channel", y="New_order_num_total", data=df)
# plt.show()

# 6. En fazla kazancı getiren ilk 10 müşteriyi sıralayınız.
# Row bazlı data olsaydı bu şekilde uygulardık, ancak burada her müşteri için tekilleştirilmiş data mevcut:
# df.groupby("master_id").agg({"New_customer_value_total": "sum"}).sort_values("New_customer_value_total", ascending=False)[:10]

df.sort_values("New_customer_value_total", ascending=False)[["master_id","New_customer_value_total"]][:10]

# 7. En fazla siparişi veren ilk 10 müşteriyi sıralayınız.
df.sort_values("New_order_num_total", ascending=False)[["master_id","New_order_num_total"]][:10]


# 8. Veri ön hazırlık sürecini fonksiyonlaştırınız.
def data_prep(dataframe):
    dataframe["New_order_num_total"] = dataframe["order_num_total_ever_online"] + dataframe["order_num_total_ever_offline"]
    dataframe["New_customer_value_total"] = dataframe["customer_value_total_ever_offline"] + dataframe["customer_value_total_ever_online"]
    date_cols = [col for col in dataframe.columns if "date" in col.lower()]
    dataframe[date_cols] = dataframe[date_cols].astype('datetime64[ns]')
    return dataframe
...
rfm=pd.DataFrame({"CustomerId": df["master_id"],
                   "Recency": (analysis_date - df["last_order_date"]).dt.days,
                   "Frequency": df["New_order_num_total"],
                   "Monetary": df["New_customer_value_total"]})
...
# segment
# about_to_sleep         1629 113.79      2.40   359.01
# at_Risk                3131 241.61      4.47   646.61
# cant_loose             1200 235.44     10.70  1474.47
# champions              1932  17.11      8.93  1406.63
# hibernating            3604 247.95      2.39   366.27
# loyal_customers        3361  82.59      8.37  1216.82
# need_attention          823 113.83      3.73   562.14
# new_customers           680  17.92      2.00   339.96
# potential_loyalists    2938  37.16      3.30   533.18
# promising               647  58.92      2.00   335.67
...
customers_related_categories = df[df["interested_in_categories_12"].str.contains("KADIN")][["master_id", "interested_in_categories_12"]]
customers_related_categories.rename(columns={"master_id": "CustomerId"}, inplace=True)
customers_related_categories.head()

high_segments.head(2)
customers_related_categories.head(2)

# Şimdi bu 2 dataframe'i merge edelim:
yeni_marka_hedef_müşteri_df = customers_related_categories.merge(high_segments, on="CustomerId", how="inner")
yeni_marka_hedef_müşteri_df.head()
yeni_marka_hedef_müşteri_df.to_csv("Projects\Week_2\yeni_marka_hedef_müşteri_id.csv", index=False)
yeni_marka_hedef_müşteri_df.shape # 2497
...
campaign_customers.to_csv("Projects\Week_2\indirim_hedef_müşteri_ids.csv", index=False)
...
def create_rfm(dataframe):
    ...
    rfm["frequency"] = dataframe["New_order_num_total"]
    rfm["monetary"] = dataframe["New_customer_value_total"]
